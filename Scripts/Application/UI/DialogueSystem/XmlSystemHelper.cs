...
public static class XmlSystemHelper
{
    #region Dialog
    /// <summary>
    /// 读取xml文件中的对话内容
    ...
    /// <param name="nodeName">对话节点名</param>
    /// <returns></returns>
    public static string GetItemFromXml(string fileName, string nodeName, string valueName = null)
    {
        XmlDocument xmlDoc = new XmlDocument();
        TextAsset tt = (TextAsset)Resources.Load(Consts.XmlDir + fileName);
        xmlDoc.LoadXml(tt.text);
        ...
        string var;
        if (valueName != null)
            var = xmlNode[0].Attributes.GetNamedItem(valueName).Value;
        else
            var = xmlNode[0].InnerText;
